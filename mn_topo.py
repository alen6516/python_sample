...
class SdnIpTopo( Topo ):
    ...
    def build( self ):
        ...
        for i in range(1, 3):
            ...
            router = self.addHost(name, cls=Router,
                                  intfDict=intfs)
        ...
        bgp = self.addHost( "bgp", cls=Router, inNamespace=False,
                             intfDict=bgpIntfs )
        
        self.addLink( bgp, s1 )

        # Connect BGP speaker to the root namespace so it can peer with ONOS
        #bgp = self.addHost( 'bgp', cls=Router, inNamespace=False, ip='10.0.1.101/24' )
        #self.addLink( s1, bgp )
